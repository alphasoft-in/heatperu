...
import { d as db, c as categories, s as subcategories, p as products, b as complaints } from './index_B4lP7dzv.mjs';
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "AdminLayout", $$AdminLayout, { "title": "Dashboard" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="mb-8"> <h2 class="text-2xl font-semibold text-gray-900">Bienvenido al Panel de Control</h2> <p class="mt-1 text-sm text-gray-500">Aquí puedes gestionar todo el catálogo de Heat Factory.</p> </div> <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-10"> ${stats.map((item) => renderTemplate`<div class="bg-white overflow-hidden shadow rounded-lg border border-gray-100 relative group"> <div class="p-5"> <div class="flex items-center"> <div class="flex-shrink-0 bg-[#f04f23] rounded-md p-3"> <svg class="h-6 w-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"${addAttribute(item.icon, "d")}></path> </svg> </div> <div class="ml-5 w-0 flex-1"> <dl> <dt class="text-sm font-medium text-gray-500 truncate">${item.name}</dt> <dd> <div class="text-3xl font-medium text-gray-900">${item.stat}</div> </dd> </dl> </div> </div> </div> <div class="bg-gray-50 px-5 py-3 border-t border-gray-100"> <div class="text-sm"> <a${addAttribute(item.href, "href")} class="font-medium text-[#0d1624] hover:text-[#f04f23] flex items-center">
Gestionar
