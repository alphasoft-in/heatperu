...
function DeleteProjectButton({ id, title }) {
  const [isOpen, setIsOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const handleDelete = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/proyectos/${id}`, {
        method: "DELETE"
      });
  ...
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    ...
    isOpen && /* @__PURE__ */ jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/30 backdrop-blur-md", children: /* @__PURE__ */ jsxs("div", { className: "bg-white rounded-xl shadow-2xl w-full max-w-sm relative overflow-hidden text-left", children: [
      /* @__PURE__ */ jsxs("div", { className: "p-6", children: [
        ...
        /* @__PURE__ */ jsx("h3", { className: "text-lg font-bold text-center text-gray-900 mb-2", children: "¿Eliminar Proyecto?" }),
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const totalItemsResult = await db.select({ value: count() }).from(projects);
  const totalItems = totalItemsResult[0].value;
  const totalPages = Math.ceil(totalItems / limit);
  const maxOrderResult = await db.select().from(projects).orderBy(desc(projects.order)).limit(1);
  const nextOrder = maxOrderResult.length > 0 ? maxOrderResult[0].order + 1 : 1;
  const dbProjects = await db.select().from(projects).orderBy(asc(projects.order), desc(projects.id)).limit(limit).offset(offset);
  return renderTemplate`${renderComponent($$result, "AdminLayout", $$AdminLayout, { "title": "Gestión de Proyectos | Heat Factory" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="mb-5"> <h2 class="text-xl font-bold text-[#0d1624]">Gestión de Proyectos</h2> <p class="mt-1 text-[13px] text-gray-500">Agrega, edita o elimina los proyectos que se muestran en la página pública de Proyectos.</p> </div> <div class="bg-white shadow rounded-lg border border-gray-100 overflow-hidden"> <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between"> <h3 class="text-base font-semibold text-slate-800">Proyectos Actuales (${totalItems})</h3> ${renderComponent($$result2, "CreateProjectModal", null, { "nextOrder": nextOrder, "client:only": "react", "client:component-hydration": "only", "client:component-path": "D:/Proyectos/heatperu/src/components/admin/CreateProjectModal.tsx", "client:component-export": "default" })} </div> <div class="p-0"> ${dbProjects.length === 0 ? renderTemplate`<p class="text-gray-500 text-sm p-6 text-center">No hay proyectos registrados aún. ¡Añade el primero!</p>` : renderTemplate`${renderComponent($$result2, "Fragment", Fragment$1, {}, { "default": async ($$result3) => renderTemplate` <div class="overflow-x-auto"> <table class="min-w-full divide-y divide-gray-200"> <thead class="bg-gray-50"> <tr> <th scope="col" class="px-5 py-3 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider w-20">Imagen</th> <th scope="col" class="px-5 py-3 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Título</th> <th scope="col" class="px-5 py-3 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Orden</th> <th scope="col" class="px-5 py-3 text-right text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Acciones</th> </tr> </thead> <tbody class="bg-white divide-y divide-gray-100"> ${dbProjects.map((project) => renderTemplate`<tr class="hover:bg-gray-50/50 transition-colors"> <td class="px-5 py-2.5 whitespace-nowrap"> <div class="flex-shrink-0 h-10 w-14 bg-white rounded border border-gray-200 overflow-hidden flex items-center justify-center shadow-sm"> ${project.imageUrl && project.imageUrl !== "/placeholder.png" ? renderTemplate`<img${addAttribute(project.imageUrl, "src")}${addAttribute(project.title, "alt")} class="h-full w-full object-cover">` : renderTemplate`<svg class="h-5 w-5 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path> </svg>`} </div> </td> <td class="px-5 py-2.5"> <div class="text-[12px] font-semibold text-slate-700 tracking-wider line-clamp-1"${addAttribute(project.title, "title")}> ${project.title} </div> </td> <td class="px-5 py-2.5 whitespace-nowrap"> <div class="text-xs text-gray-500 bg-gray-100/80 px-2 py-0.5 rounded inline-block font-mono border border-gray-200/60"> ${project.order} </div> </td> <td class="px-5 py-2.5 whitespace-nowrap text-right text-sm font-medium"> <div class="flex justify-end gap-3"> ${renderComponent($$result3, "EditProjectModal", null, { "project": project, "client:only": "react", "client:component-hydration": "only", "client:component-path": "D:/Proyectos/heatperu/src/components/admin/EditProjectModal.tsx", "client:component-export": "default" })} ${renderComponent($$result3, "DeleteProjectButton", DeleteProjectButton, { "id": project.id, "title": project.title, "client:load": true, "client:component-hydration": "load", "client:component-path": "D:/Proyectos/heatperu/src/components/admin/DeleteProjectButton.tsx", "client:component-export": "default" })} </div> </td> </tr>`)} </tbody> </table> </div> ${totalPages > 1 && renderTemplate`<div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between"> <div class="text-sm text-gray-500">
Mostrando <span class="font-medium">${offset + 1}</span> a <span class="font-medium">${Math.min(offset + limit, totalItems)}</span> de <span class="font-medium">${totalItems}</span> proyectos
...
}, "D:/Proyectos/heatperu/src/pages/admin/proyectos/index.astro", void 0);

const $$file = "D:/Proyectos/heatperu/src/pages/admin/proyectos/index.astro";
const $$url = "/admin/proyectos";
